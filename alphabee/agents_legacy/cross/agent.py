...
from alphabee.agents_legacy.cross.prompts import (
    CROSS_HARNESS_CRITIC_PROMPT,
    CROSS_HARNESS_EVALUATOR_PROMPT,
    CROSS_HARNESS_REPORTER_PROMPT,
)
from alphabee.agents_legacy.fundamental.agent import fundamental_agent_factory
from alphabee.agents_legacy.market.agent import market_agent_factory
from alphabee.agents_legacy.risk.agent import risk_agent_factory
...
async def _invoke_subagent(
    name: str, runnable, query: str, config: RunnableConfig | None = None
...
async def collect_subagent_artifacts(state: CrossAnalysisState, config: RunnableConfig) -> CrossAnalysisState:
    ...
    results = await asyncio.gather(
        *[
            _invoke_subagent(name, _SUBAGENT_RUNNABLES[name], query, config)
...
async def run_cross_harness(state: CrossAnalysisState, config: RunnableConfig) -> CrossAnalysisState:
...
async def supplement_missing_data(state: CrossAnalysisState, config: RunnableConfig) -> CrossAnalysisState:
