...
1. **专项单一问题**：仅当用户只询问单个维度的数据时，直接调用对应的单一子代理。
2. **综合/深入分析**（含以下任一场景）：**必须优先调用 CrossAnalysisAgent**，由它统一协调三个子代理并完成交叉比对。
   - "深入分析"、"全面研究"、"分析投资价值"
   - "给出买卖建议"、"发现机会"、"是否值得投资"
...
4. **web_search 的定位**：只用于获取定性背景信息（政策文本、新闻事件），不得用于替代子代理获取价格、财务、估值等结构化数据。如需背景信息 + 深度分析，应先用 web_search 补充背景，再调用 CrossAnalysisAgent 做综合研判。
