...
def get_industry_fact(symbol: str) -> dict[str, Any]:
    ...
    Returns:
        包含行业信息的字典，含申万行业分类、行业指数近期表现和估值快照。
    """
    ts_code = normalize_ts_code(symbol)
    ...
    def _compute() -> dict[str, Any]:
        ...
        # Find matching SW index for the company's industry
        sw_code = None
        sw_daily: list[dict] = []
        sw_daily_error = None
        ...
        if industry and not sw_class_df.empty:
            ...
            if name_col:
                ...
                if not matched_rows.empty:
                    sw_code = safe_str(matched_rows.iloc[0].get("index_code"))
...
def render(data: dict[str, Any]) -> str:
    ...
    if sw_classes:
        ...
        for row in sw_classes:
            idx_code = safe_str(row.get("index_code"))
            idx_name = safe_str(row.get("industry_name") or row.get("name") or "")
            lines.append(f"| {idx_code} | {idx_name} |")
        lines.append("")
    ...
    if sw_daily_error:
        ...
        for row in sw_daily:
            lines.append(
                ...
                f"| {safe_float(row.get('pct_change')):.2f} "
                f"| {safe_float(row.get('pe')):.2f} "
                f"| {safe_float(row.get('pb')):.2f} |"
            )
        lines.append("")
