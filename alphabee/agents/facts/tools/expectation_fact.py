...
def get_expectation_fact(symbol: str) -> dict[str, Any]:
    ...
    Returns:
        包含业绩预期数据的字典，含业绩预告（forecast）和业绩快报（express）。
    """
    ts_code = normalize_ts_code(symbol)
...
def render(data: dict[str, Any]) -> str:
    ...
    if forecast_error:
        ...
        for row in forecast:
            ...
            f_type = safe_str(row.get("type"))
            p_min = safe_float(row.get("p_change_min"))
            p_max = safe_float(row.get("p_change_max"))
            np_min = safe_float(row.get("net_profit_min")) / 1e8
            np_max = safe_float(row.get("net_profit_max")) / 1e8
            lines.append(
                f"| {ann_date} | {end_date} | {f_type} "
                f"| {np_min:.2f} | {np_max:.2f} "
                f"| {p_min:.2f} | {p_max:.2f} |"
            )
        ...
        for row in forecast[:4]:
            ...
            reason = safe_str(row.get("change_reason"))
            period = safe_str(row.get("end_date"))
            if summary:
                lines.append(f"- [{period}] {summary}")
            elif reason:
                lines.append(f"- [{period}] {reason}")
    ...
    if express_error:
        ...
        for row in express:
            ann_date = safe_str(row.get("ann_date"))
            end_date = safe_str(row.get("end_date"))
            revenue = safe_float(row.get("revenue")) / 1e8
            n_income = safe_float(row.get("n_income")) / 1e8
            yoy_sales = safe_float(row.get("yoy_sales"))
            yoy_np = safe_float(row.get("yoy_dedu_np"))
            eps = safe_float(row.get("diluted_eps"))
            roe = safe_float(row.get("diluted_roe"))
            lines.append(
                f"| {ann_date} | {end_date} "
                f"| {revenue:.2f} | {n_income:.2f} "
                f"| {yoy_sales:.2f} | {yoy_np:.2f} "
        ...
        lines += ["", "**快报业绩摘要：**"]
        for row in express[:4]:
            perf = safe_str(row.get("perf_summary"))
            period = safe_str(row.get("end_date"))
            if perf:
                lines.append(f"- [{period}] {perf}")
        lines.append("")
