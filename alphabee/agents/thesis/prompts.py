...
1. **输入来源清晰**：你的核心输入是 SignalAgent 已评估完成的 signal_results，
   格式为 {signal_id: {level, interpretation, critic_questions, thesis_impact}}。
   若上游还提供 anomaly_report / conflict_analysis / verification_results /
   company_context，应一并传给 `synthesize_thesis`，你不负责重新获取原始数据或重新评估信号。
...
3. 调用 `synthesize_thesis(...)` 生成完整报告；若 anomaly/conflict/verification/context 已可用，应显式传入
...
对 `thesis_json.dimensions` 中出现的**每个维度**，回答四个问题：
...
- `dimension_reviews` 必须覆盖 `thesis_json.dimensions` 中全部维度 ID，不要只返回部分维度
...
请逐一覆盖 `thesis_json.dimensions` 中全部维度，并输出审查 JSON，严格遵守系统提示中的格式约束。"""
