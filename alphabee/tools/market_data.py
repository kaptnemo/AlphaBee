...
_MARKET_DATA_CACHE: SyncTTLCache["MarketData"] = SyncTTLCache(ttl_seconds=300.0)
...
def get_market_data(symbol: str) -> MarketData:
    ...
    def _compute() -> MarketData:
        ...
        with TuShareHelper() as helper:
            daily_df = helper.daily(ts_code=ts_code, start_date=lookback, end_date=today).data
            daily_basic_df = helper.daily_basic(ts_code=ts_code, start_date=lookback, end_date=today).data
            moneyflow_df = helper.moneyflow(ts_code=ts_code, start_date=lookback, end_date=today).data
            stock_basic_df = helper.stock_basic(ts_code=ts_code, fields="ts_code,name,industry").data
