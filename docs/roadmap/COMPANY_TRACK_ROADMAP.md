...
1. **可计算优先**：本路线图只做「能落成数值/结构化标签」的部分——营收拆解数值、对标组基准、商业模式标签。定性叙事（产业链传导逻辑、驱动变量 playbook）归 `docs/roadmap/DOMAIN_CONTEXT_ROADMAP.md`，不重复建设。
...
### 2.1 与 `docs/industry/industry-context-injection-plan.md` 的关系（基线 vs 覆盖）
