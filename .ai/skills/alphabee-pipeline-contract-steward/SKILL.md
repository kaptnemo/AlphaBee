...
description: 当 AlphaBee 中某个节点、Artifact、Observation、Pydantic 模型、OrchestratorState 或提示词载荷的输出结构发生变化，或新增节点/artifact 需要确保下游消费，或审查某个产物的下游消费路径是否完整时使用本 Skill。它帮助 Agent 以”数据契约传播”的方式处理改动，避免上游结构变化后下游仍按旧结构读取，也避免产出 dead-end artifact。
...
- **新增**某个 orchestrator 节点或 artifact 类型（新生产者必须确保下游有消费者）
...
- **审查某个节点/artifact 的下游消费路径是否完整**——即使用户只是问”后续节点如何使用 X 产生的结果”，也应该在分析后主动修复发现的问题，而不是只输出分析报告
...
处理结构改动时，至少按这个顺序检查后续消费者。**特别注意：如果发现某个节点产出了 artifact 但下游节点没有消费它（dead-end artifact），这本身就是一个需要修复的问题，不应只停留在分析报告。**
...
- `alphabee/orchestrator/nodes/insights.py`
...
| 变化类型 | 必做判断 | 推荐处理方式 |
| --- | --- | --- |
| **新增生产者未连接下游（dead-end artifact）** | 新节点产出了 artifact，下游哪些节点应该消费？ | 逐一审查下游节点是否需要该数据，需要在 producer 或 shared builder 中接入，不能让 artifact 只产出不消费 |
...
- 若结构改了，必须同步检查 `synthesize_insights`、`run_thesis`、`review_thesis`、`reporter`、`gates`

### `synthesize_insights`

- 重点看 `insight_analysis` artifact（`InsightArtifact`），含 core_view / central_tension / main_driver / three scenarios / what_would_change_my_mind
- 若这个节点被新增或其 artifact 结构改变，必须确保 `run_thesis`、`generate_report`（通过 `build_report_generation_payload`）消费该 artifact
- 特别注意：该 artifact 是 LLM 生成的"观点文档"，下游节点应将其作为叙述主线使用，而不是替代确定性信号计算
...
- **不要发现 dead-end artifact（有生产者无消费者）后只输出分析报告，必须主动修复**
