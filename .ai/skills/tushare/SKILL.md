...
**技能只负责指路，不负责背书接口契约**：`references/数据接口.md` 对每个接口只给一行简介
+ 官方文档链接，参数与输出列必须打开链接核对，或用真实数据冒烟验证后再写调用代码。
踩过的坑：凭接口名猜测 `index_member_all(src="SW2021")`——该接口根本没有 `src` 参数，
输出列是 `l1_code/l1_name/.../l3_code/l3_name` 而非 `index_code/con_code`，调用直接返回空。
正确打开方式：接口名 → 打开 references 里的官方文档链接核对输入/输出/权限 → 再写调用代码。
