...
处理提交任务时，默认遵循“最小化、可审查、可回滚”的拆分原则：

- 只把语义一致、目的相同的改动放进同一个提交
- 若当前工作区包含多类改动，应优先按改动类型或功能边界拆分为多个提交
- 每次提交都应尽量缩小影响面，避免把无关改动、顺手修复、文档更新、测试补充混进同一个提交
- 若用户要求“全部改动都要提交”，也应在“全部提交”的前提下按最小单元分别提交，而不是合并成一个大提交
- 在完成提交流程前，应确保当前工作区中的全部改动都被纳入某个提交，避免遗留半完成变更

判定是否应拆分时，优先参考：

1. 改动类型是否不同（如 `fix` / `docs` / `test`）
2. 文件虽然相邻，但是否服务于不同目标
3. 某个提交是否可以被独立理解、独立回滚
...
- 若用户要求“按最小化原则分别提交全部改动”，则必须继续细分到最小合理提交单元，并覆盖工作区全部变更
...
内容要求：
- 标题和正文的内容都不要包含某一个实施文档，ROADMAP的标题，编号，或其他内部管理信息。只描述实际改动内容。
...
1. 先确定本次提交只包含一个最小、完整、可解释的改动单元
2. `git add <本次提交涉及的文件>`
3. `git commit -m "<生成的中文标题>"`
...
1. 重复执行“选定最小提交单元 → `git add` → `git commit`”
2. 每次提交后重新检查 `git status --short`
3. 直到工作区没有剩余未提交改动为止
