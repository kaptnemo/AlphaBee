...
class TestAgentComputesArYoy:
    ...
    def tushare_facts(self):
        ...
        annual = df[df["period"].str.endswith("1231")].head(2).reset_index(drop=True)
    ...
    async def test_agent_called_evaluate_derived_facts(self, agent_result):
        ...
        tool_names = [tc["name"] for m in messages if hasattr(m, "tool_calls") and m.tool_calls for tc in m.tool_calls]
        assert "evaluate_derived_facts" in tool_names, f"Agent 未调用 evaluate_derived_facts，实际调用了：{tool_names}"
        ...
        for m in messages:
            ...
            for tc in m.tool_calls:
                if tc["name"] == "evaluate_derived_facts":
                    ...
                    assert "accounts_receivable" in fact_values, "fact_values 缺少 accounts_receivable"
                    assert "accounts_receivable_prev" in fact_values, "fact_values 缺少 accounts_receivable_prev"
                    assert math.isclose(fact_values["accounts_receivable"], ar_current, rel_tol=1e-3), (
                        f"accounts_receivable 值不匹配：{fact_values['accounts_receivable']} vs {ar_current}"
                    )
                    assert math.isclose(fact_values["accounts_receivable_prev"], ar_prev, rel_tol=1e-3), (
                        f"accounts_receivable_prev 值不匹配：{fact_values['accounts_receivable_prev']} vs {ar_prev}"
                    )
                    return
        pytest.fail("未找到 evaluate_derived_facts 的工具调用记录")

    async def test_tool_response_contains_yoy_value(self, agent_result, expected_yoy, expected_yoy_str):
        """工具返回结果中应包含正确的 accounts_receivable_yoy 数值。"""
        messages = agent_result["messages"]
        tool_responses = [
            m for m in messages if isinstance(m, ToolMessage) and "accounts_receivable_yoy" in str(m.content)
        ]
    ...
    async def test_final_response_mentions_yoy(self, agent_result):
        ...
        assert any(keyword in final_msg for keyword in ["accounts_receivable_yoy", "应收账款", "同比", "增速"]), (
